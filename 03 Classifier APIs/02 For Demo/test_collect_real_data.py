...
def retrieveAccTx(acc_addr: str, isNormal: bool, startBlock = 0, endBlock = 99999999, offset = 10000) -> [{}]:
...
def retrieveAccExtraData(acc_addr: str) -> []:
    ...
    print("\n### Collecting Extra Data (min/ Max Balance)... ###\n")
    
    # TODO: use selenium to get min max balance
    ...
    extra_features = getEtherscanData(drvr, acc_addr)
    # example:
    # {
        # 'highestBalance': '0.000081990612963', 
        # 'highestBalanceDate': 'Mon 26, Dec 2022', 
        # 'lowestBalance': '0.000081990612963', 
        # 'lowestBalance_date': 'Mon 26, Dec 2022'
    # }
...
def addBackTimeDiff_features(all_features):
    # time diff between first & last transactions
    all_features['time_between_first_and_last_tx'] = (all_features['lastTransactionTime'] - all_features['firstTransactionTime']) / 1000
    
    # Convert timestamp
    if (all_features['highestBalanceDate'] == ""):
        return
    
    dt_hi = datetime.datetime.strptime(all_features['highestBalanceDate'].replace("Sept", "Sep"), "%a %d, %b %Y")
    dt_hi_tmp = calendar.timegm(dt_hi.utctimetuple())
    all_features['highestBalanceDate'] = dt_hi_tmp * 1000
    
    dt_low = datetime.datetime.strptime(all_features['lowestBalance_date'].replace("Sept", "Sep"), "%a %d, %b %Y")
    dt_low_tmp = calendar.timegm(dt_low.utctimetuple())
    all_features['lowestBalance_date'] = dt_low_tmp * 1000
    
    
    #  time diff for min/ max balance
    all_features['time_diff_between_max_balance_and_first_tx'] = all_features['highestBalanceDate'] - all_features['firstTransactionTime']
    all_features['time_diff_between_max_balance_and_last_tx'] = all_features['lastTransactionTime'] - all_features['highestBalanceDate']
    all_features['time_diff_between_min_balance_and_first_tx'] = all_features['lowestBalance_date'] - all_features['firstTransactionTime']
    all_features["time_diff_between_min_balance_and_last_tx"] = all_features['lastTransactionTime'] - all_features['lowestBalance_date']
...
# ------
def getEtherscanData(my_drvr, addr):

    try:
        retry = None
        retryCounter = 0
        
        url = f"https://etherscan.io//address-analytics?m=light&a={addr}&lg=en&cc=USD"
