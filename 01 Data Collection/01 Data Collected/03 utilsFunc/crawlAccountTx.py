...
API_KEY_LIST = ["UF99YFKUDW52Y6TU9I9C2JIBIXP5P6BN6F",
...
def main():
    ...
    print("starting writing the files...")
    
    # write the things to file
    with open(f'internal_tx_{minIdx}_{MaxIdx}.json', 'w+') as f:
...
def getAddrTxData(addr: str, normalTx: bool = True):
...
def testThreads(addr_li: []):
    ...
    for addr in tqdm(addrs):
        ...
        d = getAddrTxData(addr=acc_addr, normalTx=False)
        # print(f"Tx: <{acc_addr}>:", len(d))

        if (d == "Max rate limit reached"):
            print("!!ERROR!!")
            break
