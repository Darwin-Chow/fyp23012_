...
def main():
    ...
    addr_TX_di, addr_len = parseJson(REAL_FILE_internal)
    ...
    for addr_summary in tqdm(addr_list):
        ...
        internal_Tx_count = len(addr_tx_li)
        
        if (internal_Tx_count == 0 ):
        ...
        gas_li =[int(g['gas']) / (10 ** 9) for g in addr_tx_li]
        ...
        out_tx_percent = out_tx / (float(internal_Tx_count))
        in_tx_percent = in_tx / (float (internal_Tx_count) )
        ...
        addr_features = {
            ...
            "num_of_internal_transaction": internal_Tx_count,
...
def saveDataToCsv():
    ...
    make_csv_from_JSON_data(jsonData=All_addr_features_li, csvFilePath="addr_internal_features.csv")
